package top.cutexingluo.core.utils.se.algo.cpp.structure.tree.btree;
...
import top.cutexingluo.core.common.data.MapEntry;
...
import top.cutexingluo.core.utils.se.core.compare.XTComparator;
import top.cutexingluo.core.utils.se.core.iterator.BaseNowIterator;
import top.cutexingluo.core.utils.se.core.iterator.BaseNowNodeIterator;
...
public class BTree<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
     ...
     * 值集合构造器
     */
    protected Supplier<LinkedList<Entry<K, V>>> valuesSupplier;
     ...
     */
    @Data
    public class BNode implements IParent<BNode>, IData<List<BNode>>, IValue<List<Entry<K, V>>> {
        /**
         * 父节点
         */
        ...
        @Override
        public LinkedList<Entry<K, V>> getValue() {
            return this.values;
        }
        ...
        public Map.Entry<K, V> searchEntry(K key) {
            LinkedList<Entry<K, V>> vs = this.values;
            LinkedList<BNode> ns = this.nodes;
            if (!isLeaf()) {
...
//                    if (compare ==0) return values.get(i).put(key, t);
         ...
         * 插入并得到，返回插入成功前的数据
         */
        protected Entry<K, V> insertAndGetOld(K key, V value) {
            Entry<K, V> res = null;
            // 插入数据
            if (values.size() == 0 ||
                    comparator.compare(key, values.getLast().getKey()) > 0) {
