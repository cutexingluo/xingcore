...
import top.cutexingluo.core.utils.se.algo.node.BaseBiNodeSource;
import top.cutexingluo.core.utils.se.algo.cpp.math.XTBinarySearch;
...
public class BPlusTree<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
    ...
    public V insert(K key, V value) {
        ...
        Entry<K, V> old = temp.insert(key, value);
    ...
    public Entry<K, V> lowerEntry(K key) {
        ...
        int index = XTBinarySearch.upperBoundReversed(node.getValues(),
                0, node.values.size(), Entry::getKey,
                key, comparator);
        if (index < 0) {
    ...
    public class BNode implements BaseBiNodeSource<BNode>, IData<List<BNode>>, IValue<List<Entry<K, V>>> {
        ...
        @Override
        public List<Entry<K, V>> getValue() {
            return this.values;
        }
...
//                    System.out.println("node: " + node + " key: " + key + "minNode:" + minNode + " minKey: " + minKey);
            ...
            if (nodes != null) {
                ...
                for (int i = 0; i < values.size(); i++) {
                    ...
                    for (Entry<K, V> entry : left.values) {
                        K lk = entry.getKey();
                    ...
                    for (Entry<K, V> entry : right.values) {
                        K rk = entry.getKey();
