package top.cutexingluo.core.designtools.juc.impl;
...
public class XTRunAndCallList<V> implements Runnable, Callable<V> {
    ...
    /**
     * 构造
     * @param tasks 默认 Callable
     */
    public XTRunAndCallList(List<Callable<V>> tasks) {
        this.callableTasks = tasks;
    }

    public static <O> XTRunAndCallList<O> ofRunnable(List<Runnable> tasks) {
        return new XTRunAndCallList<>(Runnable.class, tasks);
    }

    public static <O> XTRunAndCallList<O> ofCallable(List<Callable<O>> tasks) {
        return new XTRunAndCallList<>( tasks);
    }
    ...
    public V call() throws Exception { //结果需要在本类的result获取
        if (results == null) results = new ArrayList<>(callableTasks.size());
